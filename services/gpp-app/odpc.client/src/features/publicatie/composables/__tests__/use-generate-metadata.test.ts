...
describe("useGenerateMetadata", () => {
	...
	it("calls the correct backend endpoint with document UUID", async () => {
		...
		// Should call the generate endpoint with just method: POST (no additional headers)
	...
	it("returns null when requested document UUID is not in documenten list", async () => {
		fetchSpy.mockResolvedValue({
			ok: true,
			json: () => Promise.resolve(createWooHooResponse())
		});
		...
		const { generateMetadata } = useGenerateMetadata();
		// Pass a document with a different UUID - DOC_UUID won't be found
		const doc = createDocument({ uuid: "other-uuid" });
		const result = await generateMetadata(DOC_UUID, createPublicatie(), [doc]);

		// When the requested document is not in the list, the implementation
		// cannot find a main document and returns null
		expect(result).toBeNull();
	});

	it("returns null and shows error toast when all document fetches fail (HTTP 502)", async () => {
		fetchSpy.mockResolvedValue({
			ok: false,
			status: 502,
		...
		expect(result).toBeNull();
		// The new implementation aggregates errors and shows a generic message
	...
	it("returns null and shows error toast when all document fetches fail (HTTP 503)", async () => {
		fetchSpy.mockResolvedValue({
			ok: false,
			status: 503,
	...
	it("checkAvailability sets isAvailable to true when health endpoint returns ok", async () => {
		...
		// With mocked empty config.odpcApiUrl, URL should be just the path
